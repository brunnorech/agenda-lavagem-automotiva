...
export const getDefaultHours = (startTime: string, endTime: string, washType?: CarWashType) => {
    const HOURS_NOT_AVAILABLE_IN_COMPLETE_TYPE = washType === 'complete' ? ['11:30', '17:30'] : [];
    ...
    return options.filter(option => !LUNCH_HOURS.includes(option)).filter(option => !HOURS_NOT_AVAILABLE_IN_COMPLETE_TYPE.includes(option));
  };